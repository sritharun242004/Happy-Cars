...
import { Search, Phone, Heart, ChevronDown, Menu, X } from 'lucide-react'
...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-[#F0F0F0]">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-7 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            {/* Nav items in bordered container - desktop only */}
            <div className="hidden lg:flex items-center border border-[#E8E8E8] rounded-full px-2 py-1">
              {navItems.map((item) => (
                <button
                  key={item.label}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button className="hidden sm:flex w-10 h-10 rounded-full border border-[#E8E8E8] items-center justify-center hover:bg-gray-50 transition-colors">
            ...
            <button
              ...
              className="lg:hidden w-10 h-10 rounded-full border border-[#E8E8E8] flex items-center justify-center hover:bg-gray-50 transition-colors"
            >
              {mobileMenuOpen ? <X size={20} className="text-[#515151]" /> : <Menu size={20} className="text-[#515151]" />}
            </button>
          </div>
        </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden mt-3 pb-3 border-t border-[#F0F0F0] pt-3">
            <div className="flex flex-col gap-1">
              {navItems.map((item) => (
                <button
                  ...
                  className="flex items-center justify-between px-4 py-3 text-sm font-medium text-[#3D3D3D] hover:bg-[#F6F6F6] rounded-lg transition-colors"
                >
                  {item.label}
                  {item.hasDropdown && <ChevronDown size={14} className="text-[#7A7A7A]" />}
                </button>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-[#3D3D3D] hover:bg-[#F6F6F6] rounded-lg transition-colors"
              >
                <Heart size={16} />
                Favorites
              </button>
